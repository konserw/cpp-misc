	...
	switch(ktory2){
		...
		case 4:
			...
			if(ile == stopien){//jezeli tyle pierwiastkow co stopien to rozkladamy
				...
				for(unsigned int i=0; i < pierwiastki.size(); ++i){
					...
					w.clear();
					for(unsigned int i = 1; i < podzielony.size(); ++i) w.push_back(podzielony.at(i)); //wyraz zerowy pomijamy bo to reszta z dzielenia
		...
		case 5:
			...
			move(13, 10);
			w.clear();
			for(unsigned int i = 1; i < wynik.size(); ++i) w.push_back(wynik.at(i)); //wyraz zerowy pomijamy bo to reszta z dzielenia
			for(int i = w.size() - 1; i >= 0; --i){
				if(w.at(i) == 0) continue;
				else if(w.at(i) > 0){
					if(i == 0) printw(" + %li", w.at(i));
