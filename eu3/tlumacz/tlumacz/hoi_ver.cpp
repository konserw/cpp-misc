/*
 * T³umacz - program s³u¿¹cy do t³umaczenia plików .CSV w HoI2
...
TOP:
	...
	for(;;){
		clear();
		mvprintw(5, 10, "Witaj w programie do tlumaczenia plikow .CSV z HoI2!");
...
bool wczyt(std::string path){
	...
	while(getline( plik, buff ).good()){
		...
		poz6 = buff.find(';', poz5+1);
		poz7 = buff.find(';', poz6+1);
		v_pol.push_back(buff.substr(++poz6, poz7 - poz6));
	}
...
bool zapis(std::string path){
	...
	if( plik.is_open() && plik.good())
		for(unsigned long i=0; i<desc.size(); ++i)
			plik<<desc[i]<<';'<<eng[i]<<";;;;;"<<v_pol[i]<<";;;;;X"<<std::endl;			
	else return false;
	return true;
}